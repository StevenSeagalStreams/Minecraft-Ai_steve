...
describe('computeOfflineProgress', () => {
  it('ignores gaps shorter than the minimum', () => {
    ...
    const result = computeOfflineProgress(state, 500, config);
  ...
  it('credits elapsed time at the configured efficiency', () => {
    const state = withGenerator(freshState(), 'g1', 10);
    const result = computeOfflineProgress(state, 10_000, config);
  ...
  it('caps very long absences and reports what was discarded', () => {
    ...
    const result = computeOfflineProgress(state, week, config);
  ...
  it('bounds the step count even for a maximum-length absence', () => {
    const state = withGenerator(freshState(), 'g1', 1);
    const result = computeOfflineProgress(state, config.offline.maxElapsedMs, config);
  ...
  it('is deterministic for the same inputs', () => {
    ...
    const first = computeOfflineProgress(state, 600_000, config);
    const second = computeOfflineProgress(state, 600_000, config);
  ...
  it('is exactly the online result scaled by efficiency', () => {
    ...
    const offline = computeOfflineProgress(state, 10_000, config);
    ...
    for (let index = 0; index < 10; index += 1) {
      online = tick(online, 1_000, config, { story: false });
    }
  ...
  it('lets autobuyers compound while away', () => {
    ...
    const result = computeOfflineProgress(automated, 600_000, config);
  ...
  it('holds automation when the config disables it offline', () => {
    ...
    const result = computeOfflineProgress(automated, 600_000, manual);
  ...
  it('raises efficiency with the offline bonus modifier, capped at 1', () => {
    ...
    expect(computeOfflineProgress(state, 10_000, bonusConfig).efficiency).toBeCloseTo(0.7, 10);

    const maxed = withUpgrade(state, 'slowBurn', 20);
    expect(computeOfflineProgress(maxed, 10_000, bonusConfig).efficiency).toBe(1);
  });
  ...
  it('queues story beats once, after the simulation', () => {
    ...
    const result = computeOfflineProgress(state, 60_000, config);
  ...
  it('records the credited time in stats', () => {
    const state = withGenerator(freshState(), 'g1', 1);
    const result = computeOfflineProgress(state, 120_000, config);
